...
const Cars = () => {
  ...
  const applyFilter = () => {
    ...
    const filtered = cars.slice().filter((car) => {
      ...
      return brand.includes(query) || model.includes(query) || category.includes(query) || transmission.includes(query)
    })
  ...
  }, [isSearchData]) // added dependency

  useEffect(() => {
    if (cars.length > 0 && !isSearchData) {
      applyFilter()
    }
  }, [input, cars, isSearchData]) // added isSearchData dependency

  // Initialize filteredCars if no search
  useEffect(() => {
